...
void game() {
  ...
  printf(
      ...
      "---------------------------- \n\n");
  printf(
      " NOW YOU WILL RECEIVE SOME QUESTIONS AND YOU MUST TO  ANSWER OF EATCH "
      "WITH 1,2,3,4 THAT IS TRUE \n\n");
  printf(
  ...
  printf(" SCORE : %d\n\n", score);
  printf(
      " WHAT IS THE CAPITAL OF DANEMARK ? \n 1.COPENHAGA\n 2.BRATISLAVA\n "
      "3.NICOSIA\n 4.BERLIN  \n\n");
  printf(" THE CORRECT ANSWER IS : ");
  scanf("%d", &answer);
  if (answer == 1)
...
void best_score() {
  ...
  fclose(file);
  printf(
      "---------------CONGRATULATIONS, YOU ARE ONE OF THE BEST PLAYER "
      "!----------\n\n");
...
void help() {
  ...
  printf(
      " IT'S VERY EASY TO PLAY,JUST START THE GAME AND CHOOSE THE CORRECT "
      "ANSWER \n\n");
...
void quiz_game() {
  menu();
  // this is the switch option witch is gonna offer to player the posibility to
  // enter a option
  int option = 0;
  scanf("%d", &option);
  switch (option) {
